...
const DonationList = () => {
  ...
  useEffect(() => {
    ...
    const checkAdminStatus = async () => {
      if (!currentUser) {
        navigate("/signin2"); // Redirect to login if not logged in
        return;
      }
      ...
      if (!userRef || userRef.data().role !== "admin") {
        navigate("/signin2"); // Redirect if not an admin
      }
    };

    checkAdminStatus(); // Check if user is authenticated and has admin role
